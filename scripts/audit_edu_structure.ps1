...
Write-Output "=== TOTAL: $($edu.Count) ==="
foreach ($s in $edu) {
    $hasH2 = if ($s.content -and $s.content -match '<h[1-4]') { "YES_HEADINGS" } else { "NO_HEADINGS" }
    $hasList = if ($s.content -and $s.content -match '<(ul|ol|li)') { "YES_LISTS" } else { "NO_LISTS" }
    $hasHr = if ($s.content -and $s.content -match '<hr') { "YES_HR" } else { "NO_HR" }
    Write-Output "[$($s.id)] hasImg: $([bool]$s.imageUrl) | $hasH2 | $hasList | $hasHr | Title: $($s.title)"
}
